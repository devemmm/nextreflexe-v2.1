...
function PatientsHome() {
  ...
  const { data: context, loadingGet } = useOutletContext() as any;
  const [filteredData, setFilteredData] = useState([]);

  function searchFunc(searchKey) {
    let result = patientsData.filter(
      (item) =>
        item.fname.includes(searchKey) ||
        item.lname.includes(searchKey) ||
        item.email.includes(searchKey) ||
        item.nid.includes(searchKey),
    );
  ...
  return (
    <>
      ...
      <PatientsTable
        datas={filteredData.length === 0 ? context : filteredData}
